...
# LangChain LLM (used when OPENAI_API_KEY is set)
LLM = ChatOpenAI(model="gpt-4o-mini", temperature=0.3) if USE_REAL_LLM else None
...
REFORMULATE_PROMPT = ChatPromptTemplate.from_messages([
    ("system", f"Rewrite user queries as clear, specific search queries. Expand abbreviations and informal language. One sentence only, no preamble. Consider the context of today's date {datetime.now().strftime('%Y-%m-%d')}. This is the date of the query."),
